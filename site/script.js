...
function initTabs() {
    ...
    tabButtons.forEach((button, index) => {
        button.addEventListener('keydown', function(e) {
            if (e.key === 'ArrowLeft') {
                e.preventDefault();
                const prevButton = index > 0 ? tabButtons[index - 1] : tabButtons[tabButtons.length - 1];
                prevButton.focus();
                prevButton.click();
            } else if (e.key === 'ArrowRight') {
                e.preventDefault();
                const nextButton = index < tabButtons.length - 1 ? tabButtons[index + 1] : tabButtons[0];
                nextButton.focus();
                nextButton.click();
            }
        });
...
function initBenefitsTabs() {
    const benefitTabButtons = document.querySelectorAll('.benefit-tab-btn');
    ...
    // Keyboard navigation
    benefitTabButtons.forEach((button, index) => {
        button.addEventListener('keydown', function(e) {
            if (e.key === 'ArrowLeft') {
                e.preventDefault();
                const prevButton = index > 0 ? benefitTabButtons[index - 1] : benefitTabButtons[benefitTabButtons.length - 1];
                prevButton.focus();
                prevButton.click();
            } else if (e.key === 'ArrowRight') {
                e.preventDefault();
                const nextButton = index < benefitTabButtons.length - 1 ? benefitTabButtons[index + 1] : benefitTabButtons[0];
                nextButton.focus();
                nextButton.click();
            }
        });
